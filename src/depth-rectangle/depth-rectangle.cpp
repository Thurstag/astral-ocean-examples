...
void DepthRectangleDemo::freeVulkan() {
    // Free buffers
...
void DepthRectangleDemo::createPipelines() {
    ...
    // Cache create info
    auto cache = ao::vulkan::GLFWEngine::LoadCache("data/depth-rectangle/caches/main.cache");
    ...
    this->pipelines.setBeforePipelineCacheDestruction([this, device = this->device](std::string name, vk::PipelineCache cache) {
        this->saveCache("data/depth-rectangle/caches", name + std::string(".cache"), cache);
    });
...
void DepthRectangleDemo::createVulkanBuffers() {
    ...
    for (size_t i = 0; i < this->swapchain->size(); i++) {
        vk::DescriptorBufferInfo buffer_info(this->ubo_buffer->buffer(), this->ubo_buffer->offset(i),
                                             this->ubo_buffer->size() / this->ubo_buffer->capacity());
...
void DepthRectangleDemo::beforeCommandBuffersUpdate() {
    if (!this->clock_start) {
        ...
        for (size_t i = 0; i < this->swapchain->size(); i++) {
            ...
            this->uniform_buffers[i].rotation = glm::rotate(glm::mat4(1.0f), glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f));
        }
