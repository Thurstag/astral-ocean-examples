...
namespace ao::vulkan {
	...
	public:
		...
		CommandBufferMetric(std::string unit, std::pair<std::weak_ptr<ao::vulkan::Device>, vk::QueryPool> module) : DurationMetric(unit), module(module) {
		...
		std::string toString() override {
			...
			// Get results
			ao::vulkan::utilities::vkAssert(ao::core::shared(this->module.first)->logical.getQueryPoolResults(
				this->module.second, Index, 2, results.size() * sizeof(u64), results.data(), sizeof(u64), vk::QueryResultFlagBits::e64 | vk::QueryResultFlagBits::eWait
			), "Fail to get query results");

			return fmt::format("{:.{}f} {}", (static_cast<double>(results[1]) - results[0]) * this->period / (std::nano::den / Period::den), Precision, this->unit);
		}
