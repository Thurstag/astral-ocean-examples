...
ObjFile::ObjFile() : v(0), v_size(0), v_cap(0), vt(0), vt_size(0), vt_cap(0), vn(0), vn_size(0), vn_cap(0), f(0), f_size(0), f_cap(0) {}
...
void objParseLine(ObjFile& result, const char* line) {
    if (line[0] == 'v' && line[1] == ' ') {
        ...
        if (result.v_size + 3 > result.v_cap) growArray(result.v, result.v_cap);
        ...
        if (result.vt_size + 3 > result.vt_cap) growArray(result.vt, result.vt_cap);
        ...
        if (result.vn_size + 3 > result.vn_cap) growArray(result.vn, result.vn_cap);
        ...
        while (*s) {
            ...
            if (fv == 2) {
                if (result.f_size + 9 > result.f_cap) growArray(result.f, result.f_cap);
...
bool objValidate(const ObjFile& result) {
    ...
    for (size_t i = 0; i < result.f_size; i += 3) {
        ...
        if (vi >= 0 && size_t(vi) >= v) return false;

        if (vti >= 0 && size_t(vti) >= vt) return false;

        if (vni >= 0 && size_t(vni) >= vn) return false;
    }
